...
int main(int argc, char **argv)
{
    ...
    geometry_msgs::TwistStamped vel_pose;
    vel_pose.twist.linear.x = 0;
    vel_pose.twist.linear.y = 0;
    vel_pose.twist.linear.z = 0.1;

    //send a few setpoints before starting
    for(int i = 100; ros::ok() && i > 0; --i){
        local_pos_pub.publish(pose);
        velocity_pub.publish(vel_pose);
        ros::spinOnce();
        rate.sleep();
    }
    ...
    pose.pose.position.z = FLIGHT_ALTITUDE;
    vel_pose.twist.linear.x = 0;
    vel_pose.twist.linear.y = 0.1;
    vel_pose.twist.linear.z = 0;
